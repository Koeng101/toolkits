        ...
        for prefix,suffix in overhang_set:
            for overhang in overhangs:
                if overhang != "TCAT" and overhang != reverse_complement("TCAT"):
                    checkseq = core.replace("NNNNNNNNN", barcode(length=9)).replace("nnnn", barcode(length=3) + overhang).replace("prefix",prefix).replace("suffix",suffix)
                    while check(checkseq) == False:
                        checkseq = core.replace("NNNNNNNNN", barcode(length=9)).replace("nnnn", barcode(length=3) + overhang).replace("prefix",prefix).replace("suffix",suffix)
                    cores.append("GGAG" + checkseq + "CGCT")
                else:
                    checkseq =core.replace("NNNNNNNNN", barcode(length=9)).replace("nnnn", barcode(length=3) + overhang + "A").replace("prefix",prefix).replace("suffix",suffix)
                    while check(checkseq) == False:
                        checkseq = core.replace("NNNNNNNNN", barcode(length=9)).replace("nnnn", barcode(length=3) + overhang + "A").replace("prefix",prefix).replace("suffix",suffix)
                    cores.append("GGAG" + checkseq + "CGCT")
...
a = {"fragment_name": ["core_{}-{}".format(i*4, (i*4)+3) for i,core in enumerate(final_cores)], "sequence": final_cores}
fragment_names = []
final_cores_output = []
banned = [record.id for record in SeqIO.parse("GenParts_round1.txt", "fasta")]
for i, b in enumerate(a["fragment_name"]):
    if b not in banned:
        fragment_names.append(b)
        final_cores_output.append(a["sequence"][i])
pd.DataFrame.from_dict({"fragment_name": fragment_names, "sequence": final_cores_output}).to_csv("fragments-2.csv")
